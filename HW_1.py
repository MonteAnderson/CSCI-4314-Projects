	...
	if ((sys.argv[1] == '-f' or sys.argv[1] == '-F') & (sys.argv[3] == '-s' or sys.argv[3] == '-S') & (sys.argv[5] == '-k' or sys.argv[5] == '-K')):
		...
		for i in range(0, len(concatenated_Seq) - len(kmer)):
			...
			for j in range(0, len(kmer)):
				...
				if(concatenated_Seq[Int] == kmer[j]):
					Int += 1
				else:
					seqCheck = False
					break
		...
		'''For every letter in the length of the new concatenated sequence, we have to loop through and see if our reverse complement kmer is in it.	'''
		for i in range(0, len(concatenated_Seq) - len(kmer)):
			...
			for j in range(0, len(kmer)):
				...
				if(concatenated_Seq[Int] == reverse_kmer[j]):
					Int += 1
		...
		print("File written to" + str(outFile))
